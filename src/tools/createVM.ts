...
// These helper functions are kept for future use when implementing rollback functionality
// /**
  ...
  if (failedSteps.length > 0 && state.vmCreated) {
    ...
    failedSteps.forEach((step) => {
      if (step.name === 'Hostname Configuration') {
        ...
        if (params.memory) {
          responseText += `  \`prlctl set "${state.vmName}" --memsize ${params.memory}\`\n`;
        }
        if (params.cpus) {
          responseText += `  \`prlctl set "${state.vmName}" --cpus ${params.cpus}\`\n`;
        }
        if (params.diskSize) {
          responseText += `  \`prlctl set "${state.vmName}" --device-set hdd0 --size ${params.diskSize}G\`\n`;
        }
        responseText += '\n';
      }
    });
    ...
    if (!params.fromTemplate && (params.memory || params.cpus || params.diskSize)) {
      ...
      try {
        ...
        if (params.diskSize) {
          ...
          hardwareCommands.push(
            `prlctl set "${sanitizedName}" --device-set hdd0 --size ${params.diskSize}G`
          );
        }
        state.configSteps.push({
    ...
    if (params.setHostname || params.createUser || params.enableSshAuth) {
      ...
      if (!wasVmRunning) {
        try {
          ...
          const skipMessage =
            'VM could not be started for configuration. Skipping hostname and user setup.';
      ...
      if (vmRunning) {
        ...
        if (params.createUser || params.enableSshAuth) {
          try {
            ...
            postConfigResults.push(
              `User '${username}' created with passwordless sudo and SSH access`
            );
          } catch (error: any) {
            state.configSteps.push({
              name: 'User and SSH Configuration',
