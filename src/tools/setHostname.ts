 ...
 */
async function checkVmStatus(vmId: string): Promise<{ running: boolean; accessible: boolean; error?: string }> {
  try {
 ...
 */
function createHostnameErrorResponse(vmId: string, hostname: string, error: Error, completedSteps: HostnameConfigStep[], failedStep?: HostnameConfigStep): any {
...
export async function handleSetHostname(request: CallToolRequest) {
  ...
  try {
    ...
    currentStep = { name: '/etc/hostname Configuration', completed: false, method: '/etc/hostname' };
    try {
    ...
    currentStep = { name: 'Runtime Hostname Configuration', completed: false, method: 'hostname command' };
    try {
    ...
    const criticalMethodsCompleted = completedSteps.some(s => s.method === '/etc/hostname') || 
                                   completedSteps.some(s => s.method === 'hostnamectl');
    
    // Build comprehensive response
    const statusIcon = success ? '✅' : (criticalMethodsCompleted ? '⚠️' : '❌');
    const statusText = success ? 'Success' : (criticalMethodsCompleted ? 'Partial Success' : 'Failed');
    ...
    return createHostnameErrorResponse(
      ...
      request.params.arguments?.hostname || 'unknown',
      error,
      completedSteps,
      currentStep
    );
